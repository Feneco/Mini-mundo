...
-- 			R: O código abaixo está concertado para MYSQL
--             A correção para postgresql está em outro arquivo.
...
CREATE TABLE LOCACAO
	(codLoc		INTEGER PRIMARY KEY AUTO_INCREMENT,
	 codCli 	INTEGER,
	 dtInicio 	date,
	 dtFim		date,
...
CREATE TABLE FILME
	(codFilme	INTEGER PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE FITA
	(codFita	INTEGER PRIMARY KEY AUTO_INCREMENT,
	 codFilme 	INTEGER,
	 parte 		VARCHAR(1),
	 CONSTRAINT fk_FILME_codFilme_fita
...
CREATE TABLE ATOR
	(codAtor	INTEGER PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE FILME_ATOR
	(codFilme INTEGER,
	 codAtor  INTEGER,
	 CONSTRAINT fk_FILME_codFilme_filme_ator
		FOREIGN KEY (codFilme)
		REFERENCES FILME(codFilme),
